...
public class ImageDataProvider
{
    ...
    private LinkedHashMap<Integer, ImageGroupCache> mCategoryGroupCaches = 
        new LinkedHashMap<Integer, ImageGroupCache>();
    ...
    public void loadImageGroupsAsync(
    		int category, int requestCount, boolean isRequestOldData, IDataLoadListener listener)
    {
    	if (mIsLoadingData) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				boolean hasMoreData = loadMoreImageGroupsByCategory(
						finalCategory, finalRequestCount, finalIsRequestOldData);
				
				m_loadDataHandler.obtainMessage(
						MSG_FETCH_IMAGEGROUP_LIST_COMPLETE, hasMoreData ? 1 : 0, 0).sendToTarget();
			}
    ...
    public boolean loadMoreImageGroupsByCategory(int category, int requestCount, boolean isRequestOldData)
    {
        ...
        //ArrayList<ImageGroup> groups = ImageDBLoader.loadImageGroupsByIds(ids, m_globalImageGroupCache);
        
        // Add data to cache
        cache.addGroups(groups, isRequestOldData);
        
        Log.d(TAGConstant.TAG_DATA_PROVIDER, cache.toString());
        LogUtil.i(TAGConstant.TAG_DATA_PROVIDER, 
                "Load data takes time: " + (System.currentTimeMillis() - start) + " ms");
