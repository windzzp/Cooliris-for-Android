                                                                     ...
                                                                     ViewPager.OnPageChangeListener
{
	...
	private static final String TAG_IMAGE_DETAIL		= "image_detail_activity";
	private static final String IMAGE_CACHE_DIR 		= "images";

    private static final int IMAGE_DISK_CACHE_SIZE  	= 1024 * 1024 * 60;
    private static final int SLIDESHOW_DELAY_TIME   	= 1000;
    private static final int ANIM_DURATION_LR 			= 300;
    private static final int ANIM_DURATION_IN_CENTER	= 800;
    
    private final static int MSG_SWITCH_NEXT            = 1;
    
    private final static int MENU_SET_WALLPAPER			= 1;
    private final static int MENU_SET_FAVORITE			= 2;
    private final static int MENU_DOWNLOAD_SINGLE_IMAGE = 3;
    private final static int MENU_DOWNLOAD_GROUP_IMAGES = 4;
    
    private ItemPosInfo          	m_imagePosInfo  	= null;
    private ArrayList<ImageGroup>	m_allGroups     	= null;
    private ArrayList<Image> 		m_allImages			= null;
    ...
    // Title layout
    private LinearLayout	 		m_infoLayout 		= null;
    private TextView 				m_titleView 		= null;
    private TextView				m_imageIndexView	= null;
    private TextView 				m_descriptionView 	= null;
    private TextView 				m_timeView 			= null;
    private TextView 				m_categoryView 		= null;
    private TextView 				m_tagView 			= null;
    private ScrollView				m_infoScrollview	= null;
    private LinearLayout			m_infoContainer		= null;
    
    // Prompt info layout
    private RelativeLayout 			m_promptInfoLayout 	= null;
    private TextView 				m_promptInfoNext 	= null;
    private TextView 				m_promptInfoTitle 	= null;
    ...
    private int	m_currentCategory = ImageGroupCategory.CATEGORY_ALL;
    ...
    protected void onPrepareData()
    {
		...
		m_imagePosInfo = getPosInfoByGroupIndex(groupIx, m_allGroups);
	...
	protected void onInitView()
    {
        ...
        m_pager.setCurrentItem(m_imagePosInfo.startPos);
        ...
        m_imageIndexView = (TextView) findViewById(R.id.imagegroup_item_index);
        ...
        m_infoScrollview = (ScrollView) findViewById(R.id.imagegroup_info_scrollview);
        m_infoContainer = (LinearLayout) findViewById(R.id.imagegroup_info_container);
        ...
        m_imageSwitcher.setFactory(new ViewSwitcher.ViewFactory()
		{
			...
			{
	            ImageViewEx imageView = new ImageViewEx(ImageDetailActivity.this);
    ...
    private class ImagePagerAdapter extends FragmentStatePagerAdapter {
        ...
        public Fragment getItem(int position) {
            //return ImageDetailFragment.newInstance((String) mImageFetcher.getAdapter().getItem(position));
    ...
    private void onGuesture(boolean bNext)
	{
...
//                    LogUtil.i(TAG_IMAGE_DETAIL, "Uri = " + uri + ", filePath = " + filePath);
    ...
    private void onDownloadSingleImage()
    {
    ...
    private void onDownloadGroupImages()
    {
    	ImageGroup currentGroup = getCurrentGroup();
    	final int ix = m_pager.getCurrentItem() - m_imagePosInfo.startPos;
    	LogUtil.d(TAG_IMAGE_DETAIL, "download image ix = " + ix + ", title = " + currentGroup.getTitle());
    ...
    protected void scrollToNextInternal(boolean bNext, boolean bResetItem)
	{
		...
		final ImageGroup currentGroup = groups.get(curGroupIx);
		...
		Toast.makeText(this, bNext ? R.string.menu_next : R.string.menu_previous,
				Toast.LENGTH_SHORT).show();
	}

	private ItemPosInfo getPosInfoBySelctedIndex(int pos, ArrayList<ImageGroup> girls)
    {
    ...
    {    	
		if (null != group) {
			...
			/*
			m_infoContainer.addOnLayoutChangeListener(new View.OnLayoutChangeListener() {
				@Override
				public void onLayoutChange(View v, int left, int top, int right,
						int bottom, int oldLeft, int oldTop, int oldRight, int oldBottom) {
					int defaultH = (int) Utils.dpTopixels(ImageDetailActivity.this, 80);
					int containerH = m_infoContainer.getHeight();
					int scrollViewH = m_infoScrollview.getHeight();
		            LogUtil.e(TAG_IMAGE_DETAIL, "Layout change: height = " + defaultH);
					LogUtil.e(TAG_IMAGE_DETAIL, "Layout change: m_infoScrollview.getHeight() = " + scrollViewH);
					LogUtil.e(TAG_IMAGE_DETAIL, "Layout change: m_infoContainer.getHeight() = " + containerH);
					
					LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) m_infoScrollview.getLayoutParams();
					if (containerH > defaultH) {
						params.height = defaultH;
					} else {
						params.height = android.widget.LinearLayout.LayoutParams.WRAP_CONTENT;
						//params.height = m_infoContainer.getHeight();
					}
					
					m_infoScrollview.setLayoutParams(params);
					//m_infoScrollview.postInvalidate();
					//m_infoScrollview.invalidate();
					m_infoLayout.requestLayout();
				}
			});*/
			/*
			m_infoScrollview.addOnLayoutChangeListener(new View.OnLayoutChangeListener() {
				@Override
				public void onLayoutChange(View v, int left, int top, int right,
						int bottom, int oldLeft, int oldTop, int oldRight, int oldBottom) {		            
					int defaultH = (int) Utils.dpTopixels(ImageDetailActivity.this, 80);
			...
			//LogUtil.e(TAG_IMAGE_DETAIL, "m_infoScrollview.getHeight() = " + m_infoScrollview.getHeight());
    ...
    protected void refreshTitleIndex(ImageGroup group, Image image) {
		if (null != group && null != image) {
			ArrayList<Image> images = group.getImageList();
			final int size = images.size();
			//final int curPageIx = m_pager.getCurrentItem() - m_imagePosInfo.startPos;
			final int curPageIx = images.indexOf(image);
			final String title = (curPageIx + 1) + " / " + size;

			if (!TextUtils.isEmpty(title)) {
				m_imageIndexView.setText(title);
			}
		} else {
			LogUtil.d(TAG_IMAGE_DETAIL, "The image index is invalid!!");
		}
    }
    ...
    private void showPromptInfo(boolean next) {
    	...
    	String nextInfo = getString(
				next ? R.string.msg_next_image_group : R.string.msg_previous_image_group);
	...
	private void entrySlideShowMode()
    {
    	if (!m_isSlideShowMode)
		{
    ...
    private void exitSlideShowMode()
    {
    	if (m_isSlideShowMode)
		{
    ...
    private class MenuOperationAsyncTask extends AsyncTask<Integer, Void, Integer> {
		...
		protected void onPostExecute(Integer result) {
			...
			if (!TextUtils.isEmpty(toastInfo)) {
				Toast.makeText(ImageDetailActivity.this, toastInfo, Toast.LENGTH_SHORT).show();
			}
		}
    }
}
